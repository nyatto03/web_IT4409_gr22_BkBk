...
export const createOrder = async (req, res, next) => {
    ...
    try {
        ...
        const durationInDays =
            (checkoutDate - checkinDate) / (1000 * 3600 * 24); // Chuyển đổi mili giây thành ngày
...
export const getOrders = async (req, res, next) => {
    try {
        const orders = await Order.find()
            ...
            .populate("room_id", "name description price status") // Lấy thông tin phòng
    ...
    try {
        ...
        if (updatedOrder.status === "pending") {
            if (status === "approved") {
                ...
                return res.status(400).json({
                    message:
                        "Chỉ có thể chuyển từ pending sang approved hoặc canceled.",
                });
            }
            ...
            if (status === "paid") {
                ...
                return res.status(400).json({
                    message:
                        "Chỉ có thể chuyển từ approved sang paid hoặc canceled.",
                });
            }
        } else {
            return res.status(400).json({
